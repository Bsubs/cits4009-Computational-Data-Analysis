    ...
    if (second_plot_type != "None" & limit_range) {
      ...
      second_plot <- second_plot +
        switch(second_plot_type,
               ...
               "Density Plot" = geom_density(fill = "red", alpha = 0.5),
               "Histogram" = geom_histogram(aes_string(x = x_var), binwidth = binwidth, fill = "red", alpha = 0.5)) +
      ...
      pairs(youtube_numeric[, selected_attrs])
        ...
        if(selected_plot == "Scatter") {
          if(input$aes_attr_1 == "None"){
            ...
            ggplot(youtube_numeric, mapping = aes_string(x = x_attr, y = y_attr, color = factor(youtube_numeric$country) )) + geom_point()
          }
