...
```{r}
#get only log cols
non_pred_columns <- !grepl("^pred", names(train_data))
train_clean <- train_data[, non_pred_columns]
train_clean <- subset(train_clean, select=-c(Country, channel_type, created_month))
...
if(any(is.na(train_clean))) {
  train_clean[is.na(train_clean)] <- 0  # Replacing NAs with 0, you can choose other imputation methods if you prefer
}
...
These values indicate that the first principal component explains approximately 38.11% of the total variance in the dataset, while the second principal component explains approximately 14.12% of the total variance. Together, these two components account for a total of approximately 52.23% of the total variance in the data.
...
# Start with a scatter plot of the observed data projected on the first two principal components
...
cumsum <- cumsum(pca$sdev^2 / sum(pca$sdev^2))

# Determine the number of components for 95% explained variance
n_components <- which(cumsum >= 0.95)[1]

print("Number of components for 95% explained variance: %d\n", n_components)

# Use PCA with 95% explained variance
library(FactoMineR)  # For the PCA function with proportion of inertia parameter
res.pca <- PCA(train_clean, scale.unit = TRUE, ncp = n_components, graph = FALSE)

# Get transformed training data
train_data_pca <- as.data.frame(res.pca$ind$coord)

# Transform the test data
# Note: The FactoMineR PCA function doesn't support directly transforming new data. 
# However, you can achieve this by projecting the test data onto the PCA loadings.
test_data_pca <- as.data.frame(scale(test_data, center = res.pca$call$centre, scale = res.pca$call$ecart.type) %*% t(res.pca$var$coord))
```
