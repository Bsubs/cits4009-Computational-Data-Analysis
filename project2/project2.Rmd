...
We will take an average of the highest and lowest yearly earnings to form a new feature that we will use as our target variable.
...
par(mfrow = c(1, 2))
hist(youtube$average_yearly_earnings, main = "Average Yearly Earnings", xlab = "Earnings", col = "lightblue", border = "black", breaks=50)
hist(youtube$average_yearly_earnings.log, main = "Log of Average Yearly Earnings", xlab = "Log(Earnings)", col = "lightgreen", border = "black", breaks=50)
par(mfrow = c(1, 1))
```



As the earnings is presented as a continuous variable, we will need to find a threshold to determine what is considered high and low earning. Consider the plot of the histograms below:
