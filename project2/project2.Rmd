...
youtube <- subset(youtube, select=-c(average_yearly_earnings.log, highest_yearly_earnings.log, lowest_yearly_earnings.log, highest_monthly_earnings.log, lowest_monthly_earnings.log))
table(youtube$average_yearly_earnings.binary)
```
...
cal_forward_sel <- subset(cal_clean, select=c(selected_features))
...
train_pred_log_reg_class <- predict(log_reg_forward, train_forward_sel, type = "class")
calibration_pred_log_reg_class <- predict(log_reg_forward, cal_forward_sel, type = "class")

train_pred_log_reg_pred <- predict(log_reg_forward, train_forward_sel, type = "prob")
calibration_pred_log_reg_pred <- predict(log_reg_forward, cal_forward_sel, type = "prob")
```

```{r}
performance_log_reg_forward_sel <- cal_scores(train_forward_sel, train_pred_log_reg_class, cal_forward_sel, calibration_pred_log_reg_class)
...
```{r, fig.width=10, fig.height=4}
p1 <- plot_precision_recall_vs_threshold(train_pred_log_reg_pred, train_forward_sel, "Precision and Recall vs. Threshold, Train Set")
p2 <- plot_precision_recall_vs_threshold(calibration_pred_log_reg_pred, cal_forward_sel, "Precision and Recall vs. Threshold, Cal Set")
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_roc(train_pred_log_reg_pred, train_forward_sel)
p2 <- plot_roc(calibration_pred_log_reg_pred, cal_forward_sel)
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_con_matrix(train_pred_log_reg_class, train_forward_sel)
p2 <- plot_con_matrix(calibration_pred_log_reg_class, cal_forward_sel)
grid.arrange(p1, p2, ncol=2)
```
...
train_pred_pca_class <- predict(log_reg_pca, train_pca, type = "class")
calibration_pred_pca_class <- predict(log_reg_pca, cal_pca, type = "class")

train_pred_log_reg_pred <- predict(log_reg_pca, train_pca, type = "prob")
calibration_pred_log_reg_pred <- predict(log_reg_pca, cal_pca, type = "prob")
```

```{r}
performance_log_reg_pca <- cal_scores(train_pca, train_pred_pca_class, cal_pca, calibration_pred_pca_class)
print(performance_log_reg_pca$train)
print(performance_log_reg_pca$calibration)
```

```{r, fig.width=10, fig.height=4}
p1 <- plot_precision_recall_vs_threshold(train_pred_log_reg_pred, train_pca, "Precision and Recall vs. Threshold, Train Set")
p2 <- plot_precision_recall_vs_threshold(calibration_pred_log_reg_pred, cal_pca, "Precision and Recall vs. Threshold, Cal Set")
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_roc(train_pred_log_reg_pred, train_pca)
p2 <- plot_roc(calibration_pred_log_reg_pred, cal_pca)
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_con_matrix(train_pred_pca_class, train_pca)
p2 <- plot_con_matrix(calibration_pred_pca_class, cal_pca)
grid.arrange(p1, p2, ncol=2)
```
...
train_pred_dt_class <- as_tibble(predict(dt_forward, train_forward_sel, type = "class")) %>%
  rename(".pred_class" = value)
calibration_pred_dt_class <- as_tibble(predict(dt_forward, cal_forward_sel, type = "class")) %>%
  rename(".pred_class" = value)

train_pred_dt_pred <- as_tibble(predict(dt_forward, train_forward_sel, type = "prob")) %>%
  rename(".pred_0" = `0`, ".pred_1" = `1`)
calibration_pred_dt_pred <- as_tibble(predict(dt_forward, cal_forward_sel, type = "prob")) %>%
  rename(".pred_0" = `0`, ".pred_1" = `1`)
...
```{r}
performance_dt_forward <- cal_scores(train_forward_sel, train_pred_dt_class, cal_forward_sel, calibration_pred_dt_class)
print(performance_dt_forward$train)
print(performance_dt_forward$calibration)
```

```{r, fig.width=10, fig.height=4}
p1 <- plot_precision_recall_vs_threshold(train_pred_dt_pred, train_forward_sel, "Precision and Recall vs. Threshold, Train Set")
p2 <- plot_precision_recall_vs_threshold(calibration_pred_dt_pred, cal_forward_sel, "Precision and Recall vs. Threshold, Cal Set")
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_roc(train_pred_dt_pred, train_forward_sel)
p2 <- plot_roc(calibration_pred_dt_pred, cal_forward_sel)
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_con_matrix(train_pred_dt_class, train_forward_sel)
p2 <- plot_con_matrix(calibration_pred_dt_class, cal_forward_sel)
grid.arrange(p1, p2, ncol=2)
```
...
train_pred_dt_class_pca <- as_tibble(predict(dt_pca, train_pca, type = "class")) %>%
  rename(".pred_class" = value)
calibration_pred_dt_class_pca <- as_tibble(predict(dt_pca, cal_pca, type = "class")) %>%
  rename(".pred_class" = value)

train_pred_dt_pred_pca <- as_tibble(predict(dt_pca, train_pca, type = "prob")) %>%
  rename(".pred_0" = `0`, ".pred_1" = `1`)
calibration_pred_dt_pred_pca <- as_tibble(predict(dt_pca, cal_pca, type = "prob")) %>%
  rename(".pred_0" = `0`, ".pred_1" = `1`)
...
```{r}
performance_dt_forward <- cal_scores(train_pca, train_pred_dt_class_pca, cal_pca, calibration_pred_dt_class_pca)
print(performance_dt_forward$train)
print(performance_dt_forward$calibration)
```

```{r, fig.width=10, fig.height=4}
p1 <- plot_precision_recall_vs_threshold(train_pred_dt_pred_pca, train_pca, "Precision and Recall vs. Threshold, Train Set")
p2 <- plot_precision_recall_vs_threshold(calibration_pred_dt_pred_pca, cal_pca, "Precision and Recall vs. Threshold, Cal Set")
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_roc(train_pred_dt_pred_pca, train_pca)
p2 <- plot_roc(calibration_pred_dt_pred_pca, cal_pca)
grid.arrange(p1, p2, ncol=2)
```

```{r, fig.width=8, fig.height=4}
p1 <- plot_con_matrix(train_pred_dt_class_pca, train_pca)
p2 <- plot_con_matrix(calibration_pred_dt_class_pca, cal_pca)
grid.arrange(p1, p2, ncol=2)
```
